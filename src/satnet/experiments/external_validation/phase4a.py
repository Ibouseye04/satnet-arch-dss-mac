...
class Phase4AConfig:
    """Immutable construction contract for the Phase 4A package."""
    ...
    synthetic_dataset_root: Path = Path(r"C:\Users\johns\external\satnet-10k-final-ml-datasets-v1-final")
...
def _build_graph(
    ...
    *,
    ...
    if all(position is not None and graph.nodes[index]["exists"] for index, position in enumerate(accepted_positions)):
        ...
        valid_ids = {index for index, position in enumerate(accepted_positions) if position is not None and graph.nodes[index]["exists"]}
        for link in _candidate_links_for_positions(walker, accepted_positions, valid_ids, config):
            graph.add_edge(link[0], link[1], distance_km=link[2], margin_db=link[3], link_type=link[4], link_mode=link[5])
    edge_rows = []
...
def _candidate_links_for_positions(
    config: WalkerDeltaConfig,
    positions: Sequence[SatellitePosition | None],
    valid_ids: set[int],
    phase_config: Phase4AConfig,
) -> Iterator[tuple[int, int, float, float, str, str]]:
    budget = LinkBudgetEngine()
    planes, sats = config.num_planes, config.sats_per_plane
    seen: set[tuple[int, int]] = set()
    for plane in range(planes):
        for sat in range(sats):
            current = plane * sats + sat
            if current not in valid_ids:
                continue
            for partner, link_type in (
                (plane * sats + ((sat + 1) % sats), "intra_plane"),
                (((plane + 1) % planes) * sats + sat, "seam_link" if plane == planes - 1 else "inter_plane"),
            ):
                key = tuple(sorted((current, partner)))
                if partner not in valid_ids or key in seen:
                    continue
                seen.add(key)
                first, second = positions[current], positions[partner]
                if first is None or second is None:
                    continue
                distance = _compute_distance_km(first, second)
                if distance > phase_config.max_isl_distance_km or not _check_line_of_sight(first, second):
                    continue
                mode, _, margin, viable = budget.evaluate_link(distance)
                if viable:
                    yield current, partner, distance, margin, link_type, mode
...
def _edge_failure_probability(
    ...
    """
    walker = WalkerDeltaConfig(num_planes=max(item.plane_idx for item in selected) + 1, sats_per_plane=max(item.sat_in_plane for item in selected) + 1, inclination_deg=53.0, altitude_km=550.0, phasing_factor=1, epoch=timestamp)
    budget = LinkBudgetEngine()
    unavailable = total = 0
    for offset in range(1, config.lookback_days + 1):
        ...
        for item in selected:
            orbit = _nearest_orbit(orbital_records.get(item.norad_id, ()), state_time, config.max_tle_age_hours)
            positions.append(_propagate(orbit, state_time, item.plane_idx * walker.sats_per_plane + item.sat_in_plane) if orbit else None)
        valid = {index for index, position in enumerate(positions) if position is not None and selected[index].status_available}
        for current in range(walker.total_satellites):
            plane, sat = divmod(current, walker.sats_per_plane)
            for partner in (plane * walker.sats_per_plane + ((sat + 1) % walker.sats_per_plane), ((plane + 1) % walker.num_planes) * walker.sats_per_plane + sat):
                key = tuple(sorted((current, partner)))
                if partner not in valid or current not in valid:
                    continue
                if current > partner:
                    continue
                total += 1
                first, second = positions[current], positions[partner]
                distance = _compute_distance_km(first, second)
                viable = distance <= config.max_isl_distance_km and _check_line_of_sight(first, second)
                if viable:
                    _, _, _, viable = budget.evaluate_link(distance)
                unavailable += int(not viable)
...
def _graph_snapshot(selected: Sequence[SelectedSatellite], timestamp: datetime, config: Phase4AConfig) -> tuple[dict[str, Any], nx.Graph, list[SatellitePosition | None]]:
    ...
    return {"timestamp": timestamp.isoformat().replace("+00:00", "Z"), "nodes": nodes, "edges": directed_edges, "physical_edge_count": len(edges)}, graph, positions
...
def _source_manifest(root: Path) -> dict[str, Any]:
    ...
    manifest = {"manifest_version": "phase4a.external_source_manifest.v1", "generated_at": datetime.now(timezone.utc).isoformat(), "sources": files}
    _write_json(root / "external_source_manifest.json", manifest)
    return manifest
...
def _provenance() -> list[dict[str, str]]:
    fields = [
        ...
        ("link_type_code_scaled_2", "frozen grid link type", "FROZEN_SATNET_DERIVATION", "code/2; intra=0 inter=1 seam=2", "unitless", "exact frozen edge scaling"),
...
def build_external_validation(config: Phase4AConfig) -> dict[str, Any]:
    ...
    statuses, daily_status, norad_ids = _load_status_records(root, config.start, config.end)
    orbits = _load_orbital_records(root, norad_ids, config.start, config.end)
    ...
    inventory = {"inventory_version": "phase4a.external_validation_inventory.v1", "status": "REAL-DATA EXTERNAL VALIDATION DATASET FROZEN", "model_inference_performed": False, "episode_count": len(rf_rows), "rejected_episode_count": len(rejected), "source_revisions": SOURCE_REVISIONS, "source_raw_sha256": {f'{entry["source_repository"]}:{entry["file"]}': entry["sha256"] for entry in source_manifest["sources"]}, "adapter_sha256": _adapter_sha256(), "classification_balance": audit["classification"], "regression_distribution": audit["regression"], "distribution_shift_audit": audit, "provenance_classes": [item["provenance_class"] for item in _provenance()], "external_pre_inference_bundle_sha256": _bundle_hash(root)}
