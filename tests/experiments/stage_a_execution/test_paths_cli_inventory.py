...
def test_preflight_checks_authorization_roots_dependencies_and_lock(tmp_path: Path) -> None:
    contract = load_frozen_contract(ROOT)
    roots = [tmp_path / name for name in ("generation", "replay", "acceptance")]
    plan = build_plan(contract, partition="development", operation="GENERATE", tooling_commit=TOOLING_COMMIT, tooling_inventory_hash=TOOLING_INVENTORY, generation_root=roots[0], replay_root=roots[1], acceptance_root=roots[2])
    authorization = make_authorization(contract, operation="GENERATE", partition="development", run_ids=list(contract.partitions["development"]["global_run_ids"]), generation_root=roots[0], replay_root=roots[1], acceptance_root=roots[2])
    result = run_preflight(repo_root=ROOT, contract=contract, plan=plan, authorization=authorization, tooling_commit=TOOLING_COMMIT, tooling_inventory_hash=TOOLING_INVENTORY, generation_root=roots[0], replay_root=roots[1], acceptance_root=roots[2], minimum_free_bytes=0)
...
def test_cli_status_never_lists_holdout_identities(synthetic_contract, tmp_path: Path, capsys) -> None:
    roots = [tmp_path / name for name in ("generation", "replay", "acceptance")]
    plan = build_plan(synthetic_contract, partition="development", operation="GENERATE", tooling_commit=TOOLING_COMMIT, tooling_inventory_hash=TOOLING_INVENTORY, generation_root=roots[0], replay_root=roots[1], acceptance_root=roots[2])
