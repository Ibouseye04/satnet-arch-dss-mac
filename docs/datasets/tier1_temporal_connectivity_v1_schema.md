...
This document defines the canonical schema for Tier 1 temporal connectivity datasets generated by `satnet-arch-dss`. These datasets use the Hypatia-based temporal pipeline with SGP4 orbital propagation and GCC-based threshold-breach labels.
...
| `partition_any` | int | ✓ | 1 if any sampled state breaches the partition threshold, 0 otherwise |
| `max_partition_streak` | int | ✓ | Longest consecutive run of threshold-breach sampled states |
...
| `partitioned` | int | ✓ | 1 if `gcc_frac_original < gcc_threshold`, 0 otherwise; this is threshold-based partitioning, not strict `num_components > 1` disconnectedness |
